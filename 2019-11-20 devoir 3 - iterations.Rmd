...
print(which(table(date4_n200) == 3))  #liste des dates d'anniversaire communes à 3 personnes
```
...
```{r}
f3 <- function(n, k) {
  x <- matrix(1, ncol = 2, nrow = 1)
  x <- as.data.frame(x)
  colnames(x) <- c("2", "3")
  x[,1] <- any(table(sample(365, size = n, replace = TRUE)) == k[1])
  x[,2] <- any(table(sample(365, size = n, replace = TRUE)) == k[2])
  return(x)
}
...
```{r}
f4 <- function(n, k, rep) {
  x <- matrix(1, ncol = length(k), nrow = rep)
  x <- as.data.frame(x)
  colnames(x) <- c("2", "3")
  for(i in 1:rep){
    x[i,1] <- any(table(sample(365, size = n, replace = TRUE)) == k[1])
    x[i,2] <- any(table(sample(365, size = n, replace = TRUE)) == k[2])
  }
  return(x)
}
